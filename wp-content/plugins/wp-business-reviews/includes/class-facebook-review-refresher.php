<?php
/**
 * Defines the Facebook_Review_Refresher class
 *
...
/**
 * Refreshes Facebook review components over time.
 *
 * @since 1.2.1
 */
class Facebook_Review_Refresher extends WP_Background_Process {
	/**
	...
	/**
	 * Instantiates the Facebook_Review_Refresher object.
	 *
	 * @since 1.2.1
	 *
	...
	public function __construct( $facebook_pages ) {
		...
		$this->action         = 'wpbr_refresh_facebook_reviews';
	...
	public function refresh_reviews() {
		...
		foreach ( $review_posts as $review_post ) {
			$this->refresh_review( $review_post->ID );
		}
	...
	protected function get_refresh_threshold() {
		 ...
		 *
		 * The default timestamp is set to one week ago, meaning any review that
		 * has not been refreshed in the past week should be refreshed.
		 *
		 * @since 1.2.1
		 *
		...
		$refresh_threshold = apply_filters(
			...
			strtotime( '-1 week', time() ) // Default to one week ago.
		);
